...
class RdfParser:
    ...
    def do():
        ...
        result.field_sets[Fields.TITLE] = ParseItemTitles(
            xpath=["//dcterms:title/text()", "//dcterms:alternative/text()"]
        )
        result.field_sets[Fields.SUBJECT] = ParseItem(
            xpath=["//dcterms:subject/rdf:Description/rdf:value/text()"]
        )
        result.field_sets[Fields.TYPE] = ParseItem(
            xpath=["//dcterms:type/rdf:Description/rdf:value/text()"]
        )
        result.field_sets[Fields.LANGUAGE] = ParseItem(
            xpath=["//dcterms:language/rdf:Description/rdf:value/text()"]
        )
        result.field_sets[Fields.AUTHOR] = ParseItem(
            xpath=[
                "//dcterms:creator/pgterms:agent/pgterms:alias/text()",
                "//dcterms:creator/pgterms:agent/pgterms:name/text()",
            ]
        )
        result.field_sets[Fields.BOOKSHELF] = ParseItem(
        ...
        result.field_sets[Fields.PUBLISHER] = ParseItem(
        ...
        dirs = [
            d
            for d in listdir(GutenbergCacheSettings.CACHE_RDF_UNPACK_DIRECTORY)
            if not d.startswith("DELETE")
        ]
        total = len(dirs)
        ...
        for idx, dir in enumerate(dirs):
            ...
            file_path = path.join(
                GutenbergCacheSettings.CACHE_RDF_UNPACK_DIRECTORY,
                dir,
                "pg%s.rdf" % (dir),
            )
            ...
            date_issued_x = doc.xpath(
                "//dcterms:issued/text()", namespaces=GutenbergCacheSettings.NS
            )
            num_downloads_x = doc.xpath(
                "//pgterms:downloads/text()", namespaces=GutenbergCacheSettings.NS
            )

            date_issued = (
                "1000-10-10"
                if not date_issued_x or date_issued_x[0] == "None"
                else str(date_issued_x[0])
            )
