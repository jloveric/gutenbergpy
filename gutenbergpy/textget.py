...
def _format_download_uri(index):
    ...
    for extension in extensions:
        ...
        uri = "{root}/{path}/{etextno}{extension}".format(
            root=uri_root, path=path, etextno=index, extension=extension
        )
        p = urlparse(uri)
...
def get_text_by_id(index):
    file_cache_location = os.path.join(
        GutenbergCacheSettings.TEXT_FILES_CACHE_FOLDER, str(index) + ".txt.gz"
    )
...
def strip_headers(text):
    ...
    for line in lines:
        ...
        if i <= 600:
            ...
            if any(
                line.startswith(token.encode("utf-8")) for token in TEXT_START_MARKERS
            ):
                reset = True
        ...
        if i >= 100:
            ...
            if any(
                line.startswith(token.encode("utf-8")) for token in TEXT_END_MARKERS
            ):
                footer_found = True
        ...
        if any(
            line.startswith(token.encode("utf-8")) for token in LEGALESE_START_MARKERS
        ):
            ignore_section = True
            continue
        elif any(
            line.startswith(token.encode("utf-8")) for token in LEGALESE_END_MARKERS
        ):
            ignore_section = False
            continue
