...
class SQLiteCache(Cache):
    ...
    def __insert_many_field_id(self, table, field1, field2, theSet):
        if len(theSet):
            query = "INSERT OR IGNORE INTO %s(%s, %s) VALUES (?,?)" % (
                table,
                field1,
                field2,
            )
    ...
    def create_cache(self, parse_results):
            ...
            if idx == Fields.FILES:
                ...
                self.cursor.executemany(
                    ...
                    map(
                        ...
                        parse_results.field_sets[Fields.FILES].setLinks,
                    ),
                )

            elif pt.needs_book_id():
                self.__insert_many_field_id(
                    self.table_map[idx], "name", "bookid", pt.set
                )
            else:
        ...
        for idx, book in enumerate(parse_results.books):
            ...
            book_id = idx + 1
            self.__insertLinks(
                list(map(lambda x: (x, book_id), book.authors_id)),
                "book_authors",
                "authorid",
                "bookid",
            )
            self.__insertLinks(
                list(map(lambda x: (x, book_id), book.subjects_id)),
                "book_subjects",
                "subjectid",
                "bookid",
            )
            ...
            self.cursor.execute(
                "INSERT OR IGNORE INTO books(publisherid,dateissued,rightsid,numdownloads,languageid,bookshelveid,gutenbergbookid,typeid) "
                "VALUES (?,?,?,?,?,?,?,?)",
                (
                    book.publisher_id,
                    book.date_issued,
        ...
        # noinspection PyUnresolvedReferences
        create_indices_query = open(
            SQLiteCache.DB_CREATE_CACHE_INDICES_FILENAME, "r"
        ).read()
    ...
    def query(self, **kwargs):
        ...
        helpers = [
            ...
            HelperQuery(
                ...
                (
                    "downloadlinks.downloadtypeid =  downloadlinkstype.id and downloadlinks.bookid = books.id",
        ...
        for idx, q in enumerate(runtime):
            query = "%s %s and %s in (%s) " % (
                query,
                q.query_struct[0],
                q.query_struct[1],
                ",".join(map(lambda x: "'%s'" % (str(x)), q.query_struct[2])),
            )
